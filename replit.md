...
- **Mobile App (Expo Router) Parity with Web Dashboard:** Mobile reaches feature parity with the marketplace dashboard via three new screens — `app/edit-profile.tsx` (role-aware: freelancer form covers full name, headline, bio, hourly rate, location, skills tag editor, portfolio array editor, and cover image upload via `uploadFile`; client form covers full name, company name, overview, location, and logo upload), `app/saved-jobs.tsx` (freelancer-only saved jobs list with pull-to-refresh, empty/error/loading states), and `app/jobs/[id]/applicants.tsx` (job-owner applicants list with shortlist/accept/reject actions, message conversation kickoff, in-app payment modal calling `useCreatePaymentIntent` + `useCompleteJob`, and post-payment review modal with star rating). The applicants screen enforces a client-side owner guard (`user.role === "client" && job.clientId === user.id`) before exposing privileged actions. Profile tab adds Edit Profile (all roles), Saved Jobs (freelancers), and routes My Jobs through `(tabs)/jobs?mine=1`. The job details sticky CTA shows "View proposals (N)" for owners alongside Apply/Save for freelancers and Message for everyone else. All new strings exist in both `en` and `ar` and respect RTL layout.
...
The marketplace web login (`/login`) and the mobile login screen surface a "Demo accounts" quick-fill panel with one-tap sign in for the two end-user roles:
- Client: noor@atmemly.com / client1234
- Freelancer: layla@atmemly.com / freelancer1234
Admin (admin@atmemly.com / admin1234) is intentionally NOT exposed in the client UIs — admins must type their credentials. Seeded by `artifacts/api-server/src/seed.ts` (lines 133–135).