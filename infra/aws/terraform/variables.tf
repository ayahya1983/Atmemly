...
variable "ssh_public_key" {
  description = <<EOT
OpenSSH public key string for the EC2 host. If empty, Terraform generates a
new ED25519 keypair and writes the private key to ./atmemly-ec2.pem.

The default below is the public half of the ED25519 keypair that was
originally generated by `terraform apply` and registered in AWS as the
`atmemly-key` key pair (see `aws ec2 describe-key-pairs --include-public-key
--key-names atmemly-key`). Pinning it here means subsequent operators do not
need the local `atmemly-ec2.pem` to run `terraform plan / apply`: the
key-material resources (`tls_private_key.generated`, `local_file.generated_key`)
get count = 0 and aws_key_pair.main matches what's in AWS.

The private half is NOT in this repo and is NOT in SSM. It must be obtained
out-of-band (or rotated using the procedure in DEPLOY-NOTES.md → "SSH key
handling") if SSH access is needed; SSM Session Manager works without it.
EOT
  type        = string
  default     = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILN3pVGa/4F4U37jc4RMOs2V7jyZiJ+sgVMHWrPW+JMC atmemly-key"
}
